...
export const MessageList = ({ messages, loading, handleClick }: {messages: Message[], loading: boolean, handleClick:(value:string)=>void}) => {
          ...
          {message.role === "user" ? (
            ...
            <div className="flex">
              ...
              <div className="w-full">
                {loading && message.content === "" ? <div>正在生成中...</div> :<MarkdownRender content={message.content} disabled={index!==messages.length-1} handleClick={handleClick}/>}
                {((loading && index === messages.length - 1) || t) ? null : (
                  <Icons
                    onCopy={() =>
                      {
